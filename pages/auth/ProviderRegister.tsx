...
const ProviderRegisterSchema = Yup.object().shape({
  ...
  licenseNumber: Yup.string().matches(/^[A-Z0-9]{5,10}$/, 'Invalid license number format').required('Medical license number is required'),
      ...
      <div className="max-w-md w-full mx-auto">
        ...
        <div className="bg-white p-8 rounded-xl shadow-lg border border-gray-200">
          ...
          <Formik
            ...
            onSubmit={(values) => {
              register({ name: values.name, email: values.email, specialty: values.specialty, licenseNumber: values.licenseNumber }, UserRole.PROVIDER);
            }}
          >
            {({ isSubmitting, errors, touched }) => (
              <Form className="space-y-4">
                ...
                <button type="submit" disabled={isSubmitting} className="w-full mt-2 text-white bg-accent hover:bg-accent-dark focus:ring-4 focus:ring-accent-light font-medium rounded-lg text-lg px-5 py-3 text-center transition-colors disabled:bg-gray-400 flex items-center justify-center">
