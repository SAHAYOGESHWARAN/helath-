...
var AdminRegisterSchema = Yup.object().shape({
...
var FormField = function (_a) {
    var name = _a.name, type = _a.type, label = _a.label, icon = _a.icon, error = _a.error, touched = _a.touched;
    return (<div>
        ...
        <div className="relative">
            ...
            <formik_1.Field type={type} name={name} placeholder={label} className={"w-full pl-10 pr-3 py-3 border bg-gray-50 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all ".concat(error && touched ? 'border-red-500 ring-red-500' : 'border-gray-300')}/>
        </div>
    ...
    return (<div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-lg mx-auto">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-200">
          <formik_1.Formik initialValues={{ name: '', email: '', password: '', confirmPassword: '' }} validationSchema={AdminRegisterSchema} onSubmit={function (values, _a) {
            var setSubmitting = _a.setSubmitting;
            register({ name: values.name, email: values.email, password: values.password }, types_1.UserRole.ADMIN);
            setSubmitting(false);
            navigate('/dashboard');
        }}>
            {function (_a) {
            ...
            return (<formik_1.Form className="space-y-5">
                ...
                <button type="submit" disabled={isSubmitting} className="w-full mt-4 text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-300 font-bold rounded-lg text-md px-5 py-3 text-center transition-all disabled:bg-primary-300 flex items-center justify-center">
                  {isSubmitting ? <Icons_1.SpinnerIcon /> : 'Create Admin Account'}
                </button>
              </formik_1.Form>);
        }}
          </formik_1.Formik>
