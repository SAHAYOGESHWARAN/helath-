...
var ProviderRegisterSchema = Yup.object().shape({
    ...
    return (<div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="w-full max-w-lg mx-auto">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-200">
          <formik_1.Formik initialValues={{ name: '', email: '', specialty: '', state: '', licenseNumber: '', password: '', confirmPassword: '' }} validationSchema={ProviderRegisterSchema} onSubmit={function (values, _a) {
            ...
            return (<formik_1.Form className="space-y-5">
                ...
                <FormField name="email" type="email" label="Work Email Address" icon={<Icons_1.EnvelopeIcon className="w-5 h-5"/>} error={errors.email} touched={touched.email}/>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField name="specialty" as="select" type="" label="Medical Specialty" icon={<Icons_1.AcademicCapIcon className="w-5 h-5"/>} error={errors.specialty} touched={touched.specialty}>
                        <option value="">Select Specialty</option>
                        {constants_1.MEDICAL_SPECIALTIES.map(function (s) { return <option key={s} value={s}>{s}</option>; })}
                    </FormField>
                    <FormField name="state" as="select" type="" label="State of Practice" icon={<Icons_1.MapPinIcon className="w-5 h-5"/>} error={errors.state} touched={touched.state}>
                        <option value="">Select State</option>
                        {constants_1.US_STATES.map(function (s) { return <option key={s} value={s}>{s}</option>; })}
                    </FormField>
                </div>

                <FormField name="licenseNumber" type="text" label="Medical License Number" icon={<Icons_1.BriefcaseIcon className="w-5 h-5"/>} error={errors.licenseNumber} touched={touched.licenseNumber}/>

                <FormField name="password" type="password" label="Create Password" icon={<Icons_1.LockClosedIcon className="w-5 h-5"/>} error={errors.password} touched={touched.password}/>
...
exports.default = ProviderRegister;
