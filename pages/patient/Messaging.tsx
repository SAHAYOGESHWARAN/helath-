...
const Messaging: React.FC = () => {
    ...
    }, [selectedProvider, messages, markMessagesAsRead]);
                        ...
                        {providers.map(p => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-4 border-b cursor-pointer ${selectedProvider?.id === p.id ? 'bg-primary-50' : 'hover:bg-gray-50'}`}
                                >
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                                {currentMessages.map(msg => (
                                    <div key={msg.id} className={`flex items-start gap-3 ${msg.senderId === user.id ? 'justify-end' : 'justify-start'}`}>
                                        ...
                                        <div className={`max-w-md p-3 rounded-lg shadow-sm ${msg.senderId === user.id ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-800'}`}>
                                            <p className="text-sm">{msg.text}</p>
                                            <p className="text-xs opacity-70 mt-1 text-right">{new Date(msg.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                                        </div>
                                        {msg.senderId === user.id && <img src={user.avatarUrl} alt="user avatar" className="w-8 h-8 rounded-full flex-shrink-0" />}
                                    </div>
                                ))}
