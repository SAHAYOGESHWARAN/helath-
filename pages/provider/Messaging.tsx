                ...
                <div className="w-1/3 border-r border-gray-200 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto">
                        {mockPatients.map(patient => (
                            <div key={patient.id} onClick={() => setSelectedPatient(patient)} className={`flex items-center p-4 cursor-pointer border-l-4 ${selectedPatient.id === patient.id ? 'border-primary-500 bg-primary-50' : 'border-transparent hover:bg-gray-50'}`}>
                    ...
                    <div className="flex-1 p-4 overflow-y-auto bg-gray-50 space-y-4">
                        {messages.map(msg => (
                             <div key={msg.id} className={`flex ${msg.senderId === user?.id ? 'justify-end' : 'justify-start'}`}>
                                <div className={`max-w-md p-3 rounded-lg ${msg.senderId === user?.id ? 'bg-tangerine text-white' : 'bg-white text-gray-800 shadow-sm'}`}>
                                    {msg.text}
                                </div>
                            </div>
                        ))}
