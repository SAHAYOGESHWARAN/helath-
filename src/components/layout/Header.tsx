...
import { BellIcon, ChevronDownIcon, SearchIcon, ArrowRightOnRectangleIcon as LogoutIcon, UserCircleIcon as ProfileIcon, CogIcon } from '@/components/shared/Icons';
        ...
        <div className="relative w-full max-w-xs lg:max-w-md">
           ...
           <div className="relative hidden md:block">
            ...
            <input
                ...
                name="search"
                id="search"
                className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-full leading-5 bg-white text-gray-700 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                placeholder="Search..."
            />
           </div>
           {/* Mobile Search Icon */}
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg py-1 hidden group-hover:block animate-fade-in origin-top-right z-10">
            {user?.role !== UserRole.ADMIN && (
              <Link to={`/${rolePath}/profile`} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                  <ProfileIcon className="w-5 h-5 mr-3 text-gray-400"/>
                  <span>My Profile</span>
              </Link>
            )}
            <Link to={`/${rolePath}/settings`} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
