...
// Copied and adapted from src/mockData.ts
const MOCK_USERS = [
  {
    id: 'pat1', name: 'John Doe', email: 'john.doe@email.com', password: 'Password123!', role: 'PATIENT', avatarUrl: 'https://i.pravatar.cc/150?u=pat1', dob: '1985-05-20', phone: '555-123-4567', address: '123 Health St, Wellness City, USA', state: 'California', status: 'Active', createdAt: '2023-10-01T10:00:00Z',
  ...
  { id: 'pro1', name: 'Jane Smith', email: 'jane.smith@email.com', password: 'Password123!', role: 'PROVIDER', avatarUrl: 'https://i.pravatar.cc/150?u=pro1', specialty: 'Cardiology', licenseNumber: 'G-12345', state: 'California', isVerified: true, phone: '555-987-6543', subscription: { planId: 'plan_d2', status: 'Active', renewalDate: '2025-01-01' }, status: 'Active', createdAt: '2023-09-15T10:00:00Z' },
  { id: 'adm1', name: 'Admin User', email: 'admin@novopath.com', password: 'password123', role: 'ADMIN', avatarUrl: 'https://i.pravatar.cc/150?u=adm1', phone: '555-555-1212', status: 'Active', createdAt: '2023-09-01T10:00:00Z' },
  { id: 'pat2', name: 'Alice Williams', email: 'alice.w@email.com', password: 'Password123!', role: 'PATIENT', avatarUrl: 'https://i.pravatar.cc/150?u=pat2', dob: '1992-11-12', phone: '555-234-5678', address: '456 Oak Ave, Wellness City, USA', state: 'California', status: 'Active', createdAt: '2024-01-20T10:00:00Z' },
  { id: 'pro2', name: 'David Chen', email: 'david.chen@email.com', password: 'Password123!', role: 'PROVIDER', avatarUrl: 'https://i.pravatar.cc/150?u=pro2', specialty: 'Dermatology', licenseNumber: 'G-67890', state: 'California', isVerified: false, phone: '555-876-5432', subscription: { planId: 'plan_d1', status: 'Active', renewalDate: '2025-02-01' }, status: 'Active', createdAt: '2024-02-10T10:00:00Z' },
];
...
const MOCK_REFERRALS = [
    { id: 'ref1', patientId: 'pat1', patientName: 'John Doe', referredTo: 'Dr. Evelyn Reed (Cardiology)', referredFrom: 'Dr. Jane Smith', createdAt: '2024-08-10', status: 'SENT', type: 'Outgoing', reason: 'Follow-up on EKG results', urgency: 'Routine', auditLog: [{ date: '2024-08-10', action: 'Referral Created', status: 'PENDING' }, { date: '2024-08-11', action: 'Sent via Fax', status: 'SENT' }] },
];
...
function toDynamoDBValue(value) {
    ...
    if (Array.isArray(value)) {
        // Filter out empty strings from arrays, as DynamoDB doesn't allow them
        const filteredArray = value.filter(item => item !== '');
        return { L: filteredArray.map(i => toDynamoDBValue(i)) };
    }
    if (typeof value === 'object') {
      const M = {};
      for(const key in value) {
        if (value[key] !== '') {
            M[key] = toDynamoDBValue(value[key]);
        }
      }
      return { M };
    }
...
const main = () => {
  ...
  for (const key in mockData) {
    const tableName = key.replace("MOCK_", "").toLowerCase();
    let data = mockData[key];

    // MOCK_MESSAGES is an object, not an array
    if (!Array.isArray(data)) {
        data = Object.values(data).flat();
    }

    dynamoDbData[tableName] = data.map((item) => {
      const dynamoItem = {};
      for (const prop in item) {
        const dynamoItemValue = toDynamoDBValue(item[prop]);
        if (dynamoItemValue) {
            dynamoItem[prop] = dynamoItemValue;
        }
      }
      return {
        PutRequest: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(dynamoDbData, null, 2)
  );
  console.log("dynamodb-import.json has been generated.");
};
